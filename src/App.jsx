...
const EXOSIGNAL_NASA_CACHE_KEY = 'exosignal:nasa-catalog:v1'
const EXOSIGNAL_NASA_CACHE_TTL = 24 * 60 * 60 * 1000

function getNasaCatalogCache() {
  try {
    const raw = window.localStorage.getItem(EXOSIGNAL_NASA_CACHE_KEY)
    if (!raw) return null
    const saved = JSON.parse(raw)
    if (!saved?.savedAt || !Array.isArray(saved?.planets)) return null
    if (Date.now() - saved.savedAt > EXOSIGNAL_NASA_CACHE_TTL) return null
...
function saveNasaCatalogCache(planets) {
  try {
    window.localStorage.setItem(
      EXOSIGNAL_NASA_CACHE_KEY,
      JSON.stringify({ savedAt: Date.now(), planets })
    )
  } catch {
...
async function fetchExoplanets() {
  const cached = getNasaCatalogCache()
